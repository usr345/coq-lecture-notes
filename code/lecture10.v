...
Proof.
  ...
  (* elim: s e=> [|x1 xs IH e] //=. (* case: andP=> //. *) case /andP. move=> Hlet. *)
  (* move: e. move=> /=. elim: s=> [|x1 xs] //. case: xs=> [|x2 xs] //=. *)
  (* case: andP=> //. case=> Hle Hsorted _. *)

  (* Пусть список e :: s отсортирован. Развернем определение ф-ции sorted'. Рассмотрим, как может выглядеть список s: *)
  (*                                                                  1. [::], тогда sorted' [::] вернет true. *)
  (* 2. Тогда зная, что letT e x && sorted' xs, доказать, что sorted' (x :: xs). *)


  (* elim: s=>[|x xs IH] //. move=> H=> /=. case xs=> //. *)
  (* sorted' [:: e, x & xs] -> *)
(* Доказать, что  *)
  (* sorted' (x :: xs) *)
  (* Раскрывая определение ф-ции sorted', получаем: *)
  (* Если xs имеет вид [::], то ф-ция sorted' вернет true. *)
  (* Если xs имеет вид [s :: l] *)
Admitted.
